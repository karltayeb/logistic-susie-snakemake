...
library(reticulate)
np <- import("numpy")
source_python('workflow/scripts/fitting/logistic_gibss_labf.py')
...
fit_gibss_abf <- function(X, y, L, ...){
    fit <- logistic_gibss_abf(X, np$array(y), as.integer(L), ...)
    fit$ser_fits <- NULL
    fit_np <- np_cast(fit)
...
fit <- rlang::exec(fit_gibss_abf, !!!fit_args)
