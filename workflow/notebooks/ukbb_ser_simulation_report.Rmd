...
```{r}
fits %>%
    filter(grepl('eb', method)) %>%
    hoist(fit, 'prior_variance') %>%
    ggplot(aes(x=prior_variance, color=method)) + geom_histogram()
```
...
```{r}
method_order <- c('quad_ser_L1', 'gibss_labf_L1', 'gibss_abf_L1', 'linear_susie_L1eb', 'quad_ser_L1eb', 'gibss_labf_L1eb', 'gibss_abf_L1eb')
...
res_cs %>%
    ...
    facet_wrap(vars(method), nrow=2) + geom_abline() + 
